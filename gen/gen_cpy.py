...
STATIC mp_obj_t *cast(mp_obj_t *mp_obj, const mp_obj_type_t *mp_type)
{
    ...
    if (res == NULL) nlr_raise(
        mp_obj_new_exception_msg_varg(
            &mp_type_SyntaxError, translate("Can't convert %s to %s!"), mp_obj_get_type_str(mp_obj), qstr_str(mp_type->name)));
    return res;
}
...
STATIC inline mp_lv_struct_t *mp_to_lv_struct(mp_obj_t mp_obj)
{
    ...
    if (!MP_OBJ_IS_OBJ(mp_obj)) nlr_raise(
            mp_obj_new_exception_msg(
                &mp_type_SyntaxError, translate("Struct argument is not an object!")));
...
{
    void *new_buffer = m_malloc(size, true);
...
STATIC mp_obj_t dict_to_struct(mp_obj_t dict, const mp_obj_type_t *type)
{
    ...
    for (uint i = 0; i < map->alloc; i++) {
        ...
        if (key != MP_OBJ_NULL) {
            ...
            if (dest[0]) nlr_raise(
                mp_obj_new_exception_msg_varg(
                    &mp_type_SyntaxError, translate("Cannot set field %s on struct %s!"), qstr_str(mp_obj_str_get_qstr(key)), qstr_str(type->name)));
        }
    }
    return mp_struct;
}
...
//        return (void*)mp_obj_get_int(self_in);
    ...
    if (!mp_get_buffer(self_in, &buffer_info, MP_BUFFER_READ)) {
        ...
        if (MP_OBJ_IS_TYPE(self_in, &mp_type_dict))
            return MP_OBJ_TO_PTR(self_in);
                ...
                mp_obj_new_exception_msg_varg(
                    &mp_type_SyntaxError, translate("Cannot convert '%s' to pointer!"), mp_obj_get_type_str(self_in)));
    }
    ...
    else
    {
        ...
        if (buffer_info.len != sizeof(result) || buffer_info.typecode != BYTEARRAY_TYPECODE){
            nlr_raise(
                mp_obj_new_exception_msg_varg(
                    &mp_type_SyntaxError, translate("Cannot convert %s to pointer! (buffer does not represent a pointer)"), mp_obj_get_type_str(self_in)));
        }
...
STATIC mp_obj_t mp_blob_cast(size_t argc, const mp_obj_t *argv)
{
    ...
    if (!MP_OBJ_IS_TYPE(type, &mp_type_type))
        nlr_raise(
            mp_obj_new_exception_msg(
                &mp_type_SyntaxError, translate("Cast argument must be a type!")));
...
{{
    ...
    {check_dim}
    {type} *lv_arr = ({type}*)m_malloc(len * sizeof({type}),true);
...
STATIC mp_obj_t {obj}_make_new(
    ...
    mp_map_t *kw_args)
{{
    return make_new(&lv_{obj}_create, type, n_args, args, kw_args);           
}}
"""
...
STATIC const mp_rom_map_elem_t {module_name}_globals_table[] = {{
    ...
    {blobs}
    {int_constants}
    {{ MP_ROM_QSTR(MP_QSTR_CPGlue), MP_ROM_PTR(&lv_cpglue_type)}},
}};
""".format(
        module_name = module_name,
